...
class Utorrent implements Extension
{
    ...
    public function work()
    {
        ...
        $this->cookiefile = tempnam(config('temp-dir', path('storage/tmp')), 'linfo_utorrent');

        curl_setopt_array($curl, array(
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_USERPWD => $this->connectionSettings['user'].':',
            CURLOPT_COOKIEJAR => $this->cookiefile ?: config('temp-dir', path('storage/tmp')).'/dev/null', // If tempnam fails this will fail on Windows
        ));
