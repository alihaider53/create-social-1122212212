...
class CurlDispatcher implements DispatcherInterface
{
    ...
    private $config = [
        ...
        CURLOPT_USERAGENT => 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/54.0.2840.59 Safari/537.36',
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->config[CURLOPT_RETURNTRANSFER] = 1;
        ...
        $this->config[CURLOPT_HTTPHEADER] = $headers;
        $this->config[CURLOPT_REFERER] = url();
        ...
        if (!isset($this->config[CURLOPT_COOKIEJAR])) {
            ...
            $cookies = str_replace('//', '/', config('temp-dir', path('storage/tmp')).'/embed-cookies.'.uniqid());
            ...
            $this->config[CURLOPT_COOKIE] = $cookie;
            $this->config[CURLOPT_COOKIESESSION] = true;
            $this->config[CURLOPT_COOKIEJAR] = $cookies;
            $this->config[CURLOPT_COOKIEFILE] = $cookies;
            $this->config[CURLOPT_FORBID_REUSE] = true;
            $this->config[CURLOPT_FRESH_CONNECT] = true;
        }
    }
