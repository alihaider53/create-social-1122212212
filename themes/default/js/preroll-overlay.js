/**
 * Created by pRocrea8 on 12/4/2017.
 */
/**
 * videojs-overlay
 * @version 1.1.4
...
(function(window, videojs) {
  ...
  var defaults = {
    ...
    repeatAd: true, //Show the ad only once or after every conten
  ...
  prerollPlugin = function(options) {
    ...
    player.on('readyforpreroll', function() {
      ...
      if(settings.href !== ''){
        ...
        blocker.onclick = function() {
            ...
            //creating click count
		$.ajax({url: settings.click, success: function(result){	}});
    ...
    player.preroll.exitPreroll = function() {
		$.ajax({url: settings.countView, success: function(result){}});
        if (player.preroll.exitPreroll){
            $.ajax({
              url: parent.baseUrl + "ads/video/get/overlay",
                type: "GET",
                dataType: "JSON",
                success: function(data) {
					if(data.length) {
						var image = data[0].image;
						var adlink = data[0].link;
						player.overlay({
							content: '<button id="close" onclick="hideOverlay()">X</button><a href="'+ adlink +'" class="ads-click" data-id="'+ data[0].ads_id +'" target="_blank"><img src="'+ image +'" width="100%" height="75px"></a>',
							debug: false,
							overlays: [ {
									start: 3,
									end: 'pause',
      ...
      player.ads.endLinearAdMode();
        //window.alert('Main Video Resumes');
